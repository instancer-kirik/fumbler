...
BEGIN
  ...
  result := jsonb_build_object('meta', raw_data -> 'meta', 'profile', raw_data -> 'profile');

  FOR section_key IN SELECT * FROM jsonb_object_keys(section_key_map) LOOP
    -- Default sizing to 'express' (most private); everything else defaults to 'matches'
    section_vis_level := COALESCE(section_vis ->> section_key, CASE WHEN section_key = 'sizing' THEN 'express' ELSE 'matches' END);
